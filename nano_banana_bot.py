...
# Модель для генерации изображения по промпту
IMAGE_GEN_MODEL = "stabilityai/stable-diffusion-3" 
...
async def get_photo(update: Update, context) -> int:
    ...
    # Скачиваем файл в память, а не на диск
...
async def generate_image_with_openrouter(update: Update, context) -> int:
    ...
    try:
        ...
        # Запрос к Gemini для создания промпта
        payload_gemini = {
        ...
        await query.edit_message_text(text=f"⏳ Отлично! Отправляю запрос на генерацию нового образа... Это может занять минуту.")
        ...
        response_image_gen = requests.post(
            ...
            timeout=120 # Увеличиваем таймаут для генерации изображения
        )
        ...
        error_message = f"😔 Ошибка при связи с API. Возможно, модель занята или ваш баланс на OpenRouter исчерпан. Попробуйте позже.\n\nДетали: {e}"
        print(error_message)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        conversation_timeout=600 # Диалог автоматически прервется через 10 минут
    )
